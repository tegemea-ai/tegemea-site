...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentView }) => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || currentView !== 'home' || isMenuOpen ? 'bg-white/95 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          <div className="flex items-center space-x-2 cursor-pointer z-50" onClick={() => handleMobileNav('home')}>
            <span className={`text-2xl font-serif font-bold tracking-tight transition-colors duration-300 ${isMenuOpen ? 'text-brand-navy' : 'text-brand-navy'}`}>TEGEMEA</span>
          </div>
          ...
          {/* Mobile Menu Button */}
          <button 
            className="md:hidden text-brand-navy z-50 p-2"
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-white z-[40] transition-transform duration-500 ease-in-out transform ${isMenuOpen ? 'translate-y-0' : '-translate-y-full md:hidden'}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-8 px-6 text-center">
          <button 
            onClick={() => handleMobileNav('home')}
            className={`text-2xl font-serif ${currentView === 'home' ? 'text-brand-gold' : 'text-brand-navy'}`}
          >
            Home
          </button>
          <button 
            onClick={() => handleMobileNav('services')}
            className={`text-2xl font-serif ${currentView === 'services' ? 'text-brand-gold' : 'text-brand-navy'}`}
          >
            Services
          </button>
          <button 
            onClick={() => handleMobileNav('join')}
            className={`text-2xl font-serif ${currentView === 'join' ? 'text-brand-gold' : 'text-brand-navy'}`}
          >
            Join Us
          </button>
          ...
          <div className="pt-12">
            ...
            <a href="mailto:info@tegemea.com" className="text-sm text-brand-navy hover:text-brand-gold transition-colors">info@tegemea.com</a>
          </div>
        </div>
      </div>
    </>
